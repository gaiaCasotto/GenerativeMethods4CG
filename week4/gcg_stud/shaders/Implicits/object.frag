...
vec3 dist_grad(vec3 p) {  //part 1
    ...
    float dx = (dist(vec3(p.x + delta, p.y, p.z)) - dist(vec3(p.x - delta, p.y, p.z))) * two_delta;
    float dy = (dist(vec3(p.x, p.y + delta, p.z)) - dist(vec3(p.x, p.y - delta, p.z))) * two_delta;
    float dz = (dist(vec3(p.x, p.y, p.z + delta)) - dist(vec3(p.x, p.y, p.z - delta))) * two_delta;
