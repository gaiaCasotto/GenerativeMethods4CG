from numpy import eye, array, zeros, deg2rad
...
def transform(sstr):
    ...
    for sym in sstr:
        match sym:
            case 'A', s, w:  #the symbol is smt like (A, 100, w0)
                # Part 1:
                # Implemenet the production rule in Sec. 6.3 of Prusinkiewicz et al.
                new_sstr += [('!', w), ('F', s), ('['), ('+', alpha1, '/', phi1), ('A', s*r1,  w*(q**e)), (']'), ('['), ('+', alpha2, '/', phi2), ('A', s*r2, w*((1-q)**e)), (']')]
            case _:
                new_sstr.append(sym)
    return new_sstr
...
class SpaceTurtle:
    ...
    def __init__(self):
        self.T = eye(4,4) #transform : rotation + translation matrix
        self.w = 0    #weight of line
        self.Tw_stack = []   #for branches
    ...
    def push(self):
        ...
        # Implement this function
        self.Tw_stack.append((self.T.copy(), self.w))
    ...
    for sym in sstr:
        ...
        match sym:
            case 'F', d:
                ...
                edges.append((v,vo))  #[(the current size, the size before), (), ....]
                radii[v] = turtle.w  
            case '!', w:
                turtle.set_width(w)
            #case 'A', w, s:  
            case '+', alpha, '/', phi:
                turtle.turn(deg2rad(alpha))
                turtle.roll(deg2rad(phi))
            case '[':
                turtle.push()
                current_pos = turtle.get_pos()
